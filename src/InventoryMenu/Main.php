            ...
            if ($packet->windowid == 10 && $this->chest[$player->getName()] && $packet->item->getId() == 0) {
                ...
                switch ($this->chest[$player->getName()][0]) { //chest id
                    case 1:
                        switch ($packet->slot) {
                            case 0:
                                $player->sendMessage("You have got starter crate!");
								   $command = "crate starter";
                                   $this->owner->getServer()->getCommandMap()->dispatch($event->getPlayer(), $command);
                                break;
                            case 1:
                                $player->sendMessage("You have got monthly crate!");
								   $command = "crate monthly";
                                   $this->owner->getServer()->getCommandMap()->dispatch($event->getPlayer(), $command);
                                break;
                        }
                        break;
                    case 2:
                        switch ($packet->slot) {
                            case 0:
                                ...
                                break;
                            case 1:
                                $player->sendMessage("§Buycraft: (link) \n§eBuy ranks to support the server!");
                                break;
                        }
                        break;
                }
